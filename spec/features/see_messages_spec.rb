#feature 'See messages' do
 ...
 feature 'See messages' do
  scenario 'user can see messages on the page' do
    ...
    connection.exec("INSERT INTO messages_test(message) VALUES ('We are learning how to build a full-stack application');") # add test1 message to db
    connection.exec("INSERT INTO messages_test(message) VALUES ('From Monday we will no longer be the newest cohort');") # add test2 message to db
    connection.exec("INSERT INTO messages_test(message) VALUES ('Tweets sound better than peeps');") # add test3 message to db
    visit '/index'
